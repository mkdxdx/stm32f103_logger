...
#define FCLK_SLOW() { SD_IF->CR1 = (SD_IF->CR1 & ~0x38) | SPI_BaudRatePrescaler_64; }	// Set SCLK = PCLK / 64 */
#define FCLK_FAST() { SD_IF->CR1 = (SD_IF->CR1 & ~0x38) | SPI_BaudRatePrescaler_2; }	// Set SCLK = PCLK / 2 */

#define CS_LOW()		GPIO_WriteBit(SD_GPIO,SD_Pin_CS,Bit_RESET)
#define CS_HIGH()    	GPIO_WriteBit(SD_GPIO,SD_Pin_CS,Bit_SET)

#define SD_IF_RCC		RCC_APB2Periph_SPI1

#define SD_GPIO			GPIOA
#define SD_GPIO_RCC		RCC_APB2Periph_GPIOA
#define SD_Pin_MOSI		GPIO_Pin_7
#define SD_Pin_MISO		GPIO_Pin_6
#define SD_Pin_SCK		GPIO_Pin_5
#define SD_Pin_CS		GPIO_Pin_4
...
void init_spi (void)
{
	...
	RCC_APB2PeriphClockCmd(SD_IF_RCC,ENABLE);
	RCC_APB2PeriphClockCmd(SD_GPIO_RCC,ENABLE);

	GPIO_StructInit(&gis);
	gis.GPIO_Pin = SD_Pin_MOSI | SD_Pin_SCK;
	gis.GPIO_Mode = GPIO_Mode_AF_PP;
	gis.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SD_GPIO,&gis);

	GPIO_StructInit(&gis);
	gis.GPIO_Pin = SD_Pin_MISO;
	gis.GPIO_Mode = GPIO_Mode_IPU;
	gis.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SD_GPIO,&gis);

	GPIO_StructInit(&gis);
	gis.GPIO_Pin = SD_Pin_CS;
...
)
{
	...
	SPI_DataSizeConfig(SD_IF, SPI_DataSize_16b);

	SD_IF->DR = 0xFFFF;
	btr -= 2;
	do {					/* Receive the data block into buffer */
		while (SPI_I2S_GetFlagStatus(SD_IF,SPI_I2S_FLAG_BSY) == SET) ;
		d = SD_IF->DR;
		SD_IF->DR = 0xFFFF;
		buff[1] = d;
		buff[0] = d >> 8;
	...
	SPI_DataSizeConfig(SD_IF, SPI_DataSize_8b);
}
...
void disk_timerproc (void)
{
	...
	// Next section is not used - module has no pin to detect if card is present
	/*
